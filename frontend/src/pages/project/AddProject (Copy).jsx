...
export default function AddProjectModal({ onClose, onSuccess }) {
  ...
  const loadProjects = async () => {
    try {
      ...
      console.error("Failed to load parent projects");
    }
  };
  ...
  const submit = async (e) => {
    ...
    if (!form.project_name || !form.location || !form.start_date) {
      setError("Project name, location and start date are required");
      return;
    }
          ...
          {form.status === "Hold" && (
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                className="w-full border px-3 py-2 rounded"
                placeholder="Why project is on hold?"
              />
            </div>
          )}
